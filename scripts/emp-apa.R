...
apa.table <- ggplot_gtable(ggplot_build(apa.plot))
## remove elements (trial and error)
wh_rm <- c(3, 32)
apa.table$grobs <- apa.table$grobs[-wh_rm]
apa.table$layout <- apa.table$layout[-wh_rm, ]
## move axis element (trial and error)
wh_mv <- 13
apa.table$layout[wh_mv, c("t", "b")] <-
  apa.table$layout[wh_mv, c("t", "b")] / 1.5
# Plots
